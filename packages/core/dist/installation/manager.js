...
export class InstallationManager {
    ...
    async install() {
        ...
        try {
            ...
            const prerequisites = await this.executeStep(InstallationStep.CHECK_PREREQUISITES, async () => {
            ...
            await this.executeStep(InstallationStep.INSTALL_PREREQUISITES, async () => {
                await this.installPrerequisites(prerequisites);
            });
    ...
    async executeStep(step, action) {
        ...
        try {
            ...
            this.updateProgress(step, StepStatus.FAILED, `${step} failed: ${error.message}`, this.calculateProgress(step), error);
            throw error;
        }
    }
    /**
     ...
     */
    updateProgress(step, status, message, progress, error) {
     ...
     */
    async checkCommand(command, name, required, requiredVersion) {
        try {
     ...
     */
    async installPrerequisites(checks) {
     ...
     */
    async installPrerequisite(name) {
...
`;
    ...
    async setupSSL() {
        ...
        const domain = this.options.config.addon.domain;
        // Run certbot
        const result = await this.execSudo(`certbot --nginx -d ${domain} --non-interactive --agree-tos --email admin@${domain} --redirect`);
        if (result.code !== 0) {
